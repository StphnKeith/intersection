...
typedef struct _SafeStopSign {
	...
	*/
	...
	// Mutex and CV for letting cars into the intersection.
    pthread_mutex_t pathMutex;
    pthread_cond_t pathCV;

    // Mutex's, CV's, and Queues for letting only the first car in a lane
    // through the intersection.
    pthread_mutex_t laneMutex[QUADRANT_COUNT];
    pthread_cond_t laneCV[QUADRANT_COUNT];
    Queue laneQueue[QUADRANT_COUNT];

    // Shadow quadrant field for avoiding collisions.
